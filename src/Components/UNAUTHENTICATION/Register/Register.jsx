...
const Register = () => {
    ...
    const handleRegister = async (event) => {
        ...
        try {
            ...
            const imgRes = await fetch("https://api.cloudinary.com/v1_1/daasda9rp/image/upload", {
                method: "POST",
                body: imgData,
            });
            console.log(imgRes);
            ...
            if (profile_img.size > 5 * 1024 * 1024) { // 5MB max size
            ...
            if (response.ok) {
                ...
                toast.error(result.message || result.error || result.detail || "Registration failed");
            }
        } catch (error) {
                ...
                <div className="container mx-auto max-w-screen-xl px-6 py-3 flex flex-col md:flex-row justify-center items-center gap-10 pt-28">
                    ...
                    <div className="w-full md:w-1/2 p-6 bg-white rounded-xl shadow-lg">
                        ...
                        <form onSubmit={handleRegister} className="space-y-4">
                            ...
                            <input type="text" name="last_name" placeholder="Last Name" required className="input input-bordered w-full" />
                            <div className="relative">
                                ...
                                <span className="absolute right-3 top-3 cursor-pointer" onClick={() => togglePassword("password")}>{showPassword ? "👁️" : "🙈"}</span>
                            </div>
                            <div className="relative">
                                <input type={showConfirmPassword ? "text" : "password"} name="confirm_password" placeholder="Confirm Password" required className="input input-bordered w-full" />
